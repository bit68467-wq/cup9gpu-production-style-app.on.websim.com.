...
(function(){
  ...
  // Accept only explicit "true" or "false" commands like:
  //  "tasto otp true, valido per depositi per utente (email@dominio)"
  //  "tasto otp false, valido per depositi per utente (email@dominio)"
  //
  // Behavior:
  //  - "true" sets the per-email+tipo key to "armed"
  //  - "false" sets the per-email+tipo key to "false"
  //  - no other semantics or auxiliary logic remain here (one-shot enforcement happens on generation)
  ...
  window.CUP9.handleOtpCommand = function(cmd){
    try{
      ...
      // require "tasto otp" and either " true" or " false"
      if(!lower.includes('tasto otp')) return false;
      const isTrue = lower.includes(' true');
      const isFalse = lower.includes(' false');
      if(!isTrue && !isFalse) return false;

      // Determine tipo (deposito / prelievo) in a minimal robust way
      ...
      // Refresh UI for this email so the Generate OTP button updates
